...
; Global $DefaultTest = _StartTTS()

;~ If Not IsObj($DefaultTest) Then
;~     MsgBox(0, 'Error', 'Failed create TTS Sound Speaker object')
;~     Exit
;~ EndIf
...
ConsoleWrite('@@ Trace(324) :    		_WinAPI_UnhookWindowsHookEx($ah_MouseKeyboard_WinHooks[2])'  & @CRLF) ;### Trace Console
...
ConsoleWrite('@@ Trace(332) :    		_WinAPI_UnhookWindowsHookEx($ah_MouseKeyboard_WinHooks[3])'  & @CRLF) ;### Trace Console
...
ConsoleWrite('@@ Trace(340) :    		DllCallbackFree($ah_MouseKeyboard_WinHooks[0])'  & @CRLF) ;### Trace Console
...
ConsoleWrite('@@ Trace(348) :    		DllCallbackFree($ah_MouseKeyboard_WinHooks[1])'  & @CRLF) ;### Trace Console
