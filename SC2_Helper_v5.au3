...
If Not IsObj($Default) Then
    ...
    Exit
EndIf
_Speak2($Default, 'SC2 Ready', True) ; GegaSC2
...
Func _SC2_HotKey_ExitScript_Func()
	...
	; Custom action here
	sc2_GameScript_EnableDisable(False)
	_Speak2($Default, 'Good Luck', True)
	; Sleep(1000)
	Exit(1)
	; Replicate User Key presses -----
    Send("{CTRLDOWN}o{CTRLUP}") ; {CTRLDOWN} {CTRLUP}
...
Func SC2_ReqNewProbe($lcnProbeNum = 0)
	...
	_Speak2($Default, $var_arrayProbes_defaultbuild[$lcnProbeNum][1], False, "", False)
	SC2_SelectGroup(10)
...
Func SC2_ReqNewArmy($lcnUnitNum = 0)
	...
	_Speak2($Default, $var_arrayArmy_defaultbuild[$lcnUnitNum][1], False, "", False)
	SC2_SelectGroup(9)
